...
par_function <- function(i,j) {
        ...
        wk.dbhs <- dcast(data=wk.neighbours, Tree_ID_IN ~ voisins, value.var= "DB2009")[,-1]
...
# Annealing algorithm -----------------------------------------------------
       ...
       write_results(results,paste("./Results/Model_Fits/","DIA_", levels(SPP)[i], modelname[j],".txt", sep=""))
